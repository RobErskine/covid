<template>
  <div>
    <section class="w-full transform mb-0 flex flex-wrap">
      <div class="p-2 lg:p-4 lg:pt-6 w-full lg:w-2/3 lg:pr-12">
        <h1 class="text-xl lg:text-5xl leading-tight">How many people in the U.S. have tested positive or been vaccinated against COVID-19?</h1>
        <p class="text-lg lg:text-2xl">Of the <strong><number :val=data.population /></strong> people who live in the United States, <strong><number :val=data.total_cases /> have tested positive</strong> for COVID-19, or <number :val=data.population_infected_pct percentage=true /> of the population.</p>

        <p class="text-lg lg:text-2xl">In the last week, there have been <number :val=data.cases_last_7d /> new cases and <number :val=data.deaths_new_7d /> deaths. In the last day, there have been <strong><number :val=data.cases_last_24h /> new cases </strong> and <strong><number :val=data.deaths_new_24h /> new deaths</strong>, <span v-if="this.trending">which thankfully, is <strong>trending downwards</strong> based on the last 7 days of data</span><span v-else>which unfortunately, is <strong>trending upwards</strong> based on the last 7 days of data</span>.</p>
        ...
        <p class="text-lg lg:text-2xl">Of those doses administered, <number :val="data.dose1_population_pct" percentage="true"/> received the 1st dose, and <number :val="data.dose2_population_pct" percentage="true"/> received the 2nd dose, meaning <strong><number :val="data.dose2"/> Americans have been vaccinated.</strong></p>

        <p class="text-gray-DEFAULT text-sm">Data presented has been collected and distributed through the Centers for Disease Control and Prevention. Each state, district, and territory is listed below.</p>
      </div>

      <div class="p-4 rounded-2xl md:p-8 br-4 bg-gray-lightest scale-95 w-full lg:w-1/3 flex flex-wrap">
        <h2 class="text-3xl w-full">{{data.location}} <br/>COVID-19 Data</h2>
        <row topic="basic" class="mb-2 border-gray-light border-b-2 w-full">
        ...
        <row topic="last7" class="mb-2 border-gray-light-border-b-2 w-full">
        ...
        <row topic="Deaths" v-if="(data.deaths_total || data.deaths_per_100k || data.deaths_new_7d)" class="mb-2 border-gray-light border-b-2 w-full">
        ...
        <row topic="Vaccines" class="w-full">
