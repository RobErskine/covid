...
exports.handler = async (event, context) => {
  ...
  var getParallel = async function() {
      ...
      // loop over first request (vaccine data)
      for(var i = 0; i < array1.length; i++){
        // loop over second request (us map data)
        for(var j = 0; j < array2.length; j++){
          // merge two endpoints together where state names match
          if(array1[i]['Location'] == array2[j]['abbr']){
            ...
            mergedArray.push({
              ...
              'dose1': array1[i]['Administered_Dose1_Recip'],
              'dose2': array1[i]['Administered_Dose2_Recip'],
